#!/usr/bin/env python3
"""
Performance Test Script for Redis, Elasticsearch, PostgreSQL System
Sử dụng JMeter để benchmark và tạo report chi tiết
"""
...
class PerformanceTester:
    ...
    def run_jmeter_test(self, threads=50, ramp_up=10, loops=10):
        ...
        cmd = [
            "jmeter",
            "-n",  # Non-GUI mode
            "-t", "jmeter_test_plan.jmx",  # Test plan file
            "-l", self.jmeter_results,  # Results file
            "-e",  # Generate HTML report
            "-o", self.html_report,  # HTML report directory
            "-JTHREAD_COUNT=" + str(threads),
            "-JRAMP_UP=" + str(ramp_up),
            "-JLOOP_COUNT=" + str(loops),
            "-JBASE_URL=" + self.base_url
        ]
        ...
        try:
            ...
            print("❌ Không tìm thấy JMeter. Hãy cài đặt JMeter trước")
            return False
    ...
    def run_full_test(self, threads=50, ramp_up=10, loops=10):
        ...
        print("🚀 Bắt đầu Performance Test Suite")
...
def main():
    parser = argparse.ArgumentParser(description='Performance Test cho Redis, Elasticsearch, PostgreSQL System')
    ...
    # Kiểm tra JMeter
    try:
        subprocess.run(['jmeter', '--version'], capture_output=True, check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("❌ JMeter chưa được cài đặt hoặc không có trong PATH")
        print("📥 Hướng dẫn cài đặt JMeter:")
        print("   1. Tải JMeter từ: https://jmeter.apache.org/download_jmeter.cgi")
        print("   2. Giải nén và thêm bin/ vào PATH")
        print("   3. Hoặc chạy: export PATH=$PATH:/path/to/jmeter/bin")
        return
